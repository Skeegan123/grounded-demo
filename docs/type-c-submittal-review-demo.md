...
The prompt names the review goal and the human judgment boundary. It does not
name the mismatch, target sheets, affected rooms, or final disposition.
...
6. `create_assistance_request` with A1.2 as the Point Set target, A4.3 and the
   ...
   page
7. `get_assistance_request` after the human response
...
The human marked WC, Utility, and Coats and submitted one final three-point
...
note. Combining that response with the construction mismatch supports a
